...
class InvertedIndex:
    ...
    def __init__(self, index_dir: Path) -> None:
        ...
        if self._doc_id_map_fp.exists():
            with open(self._doc_id_map_fp, 'r') as f:
                ...
                self._doc_id_to_url = {int(k): v for k, v in json.load(f).items()}
    ...
    def _search_term(self, term: Term) -> PostingList:
        ...
        with open(self._index_fp, "rb") as f:
            ...
            return index.get(term, PostingList()) # return empty PostingList if term isnt found
    ...
    def retrieve(self, query: str) -> list[str]:
        ...
        result_doc_ids = set(posting.doc_id for posting in posting_lists[0]._postings)

        # intersect with the document IDs from the remaining posting lists
        for i in range(1, len(posting_lists)):
            next_doc_ids = set(posting.doc_id for posting in posting_lists[i]._postings)
            result_doc_ids.intersection_update(next_doc_ids) #updates set in place to contain only common elements

        #at this point result_doc_ids is a set of doc_ids that contain all terms in the query

        # convert doc_ids to urls
        retrieved_urls = [self._doc_id_to_url.get(doc_id) for doc_id in sorted(list(result_doc_ids))]
